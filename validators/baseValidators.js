...
let price = (price) => {
    ...
    if (validator.isCurrency(price + '', { require_symbol: false, allow_negatives: false /*, digits_after_decimal: [2] */})) {
        return true;
    }
    else {
...
let positiveNumber = (number) => {
    ...
    // check if number is numeric
    if (validator.isNumeric(number + '', { no_symbols: true, allow_negatives: false })) {
        return true;
    }
    else {
